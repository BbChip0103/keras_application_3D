...
from .custom_layers import DepthwiseConv3D
...
def block3(x, filters, kernel_size=3, stride=1, group_base_channel=64, groups=32,
    ...
    c = filters // groups
    x = DepthwiseConv3D(kernel_size, strides=stride, depth_multiplier=c, padding='same',
                               use_bias=False, name=name + '_2_conv')(x)
    ...
    x = layers.Conv3D(filters, 1, use_bias=False, trainable=False, padding='same', 
...
def ResNet(stack_fn,
    ...
    ### ???
    x = layers.Conv3D(first_num_of_conv, 7, strides=2, padding='same', 
                        use_bias=use_bias, name='conv1_conv')(img_input)
...
def CustomResNetHead(stack_fn,
    ...
    # x = layers.Conv3D(first_num_of_conv, 7, strides=2, use_bias=use_bias, name='conv1_conv')(x)
    ...
    def stack_fn(x):
        ...
        for depth_i in range(1, len(num_of_block)):
            x = stack1(x, base_channel*(2**(depth_i+1)), num_of_block[depth_i], 
                        name='conv{}'.format(depth_i+2))
        return x
    return ResNet(stack_fn, False, True, 'resnet_3D_custom_{}'.format(base_channel),
                  include_top, weights,
                  input_tensor, input_shape,
                  pooling, classes,
                  first_num_of_conv=base_channel, 
...
def ResNet3DV2(include_top=True,
    ...
    def stack_fn(x):
        ...
        x = stack2(x, base_channel*(2**(depth_i+1)), num_of_block[depth_i], stride1=1, 
                    name='conv{}'.format(depth_i+2))
        return x
    return ResNet(stack_fn, False, True, 'resnet_3D_V2_{}'.format(base_channel),
                  include_top, weights,
                  input_tensor, input_shape,
                  pooling, classes,
                  first_num_of_conv=base_channel, 
...
setattr(ResNet3D, '__doc__', ResNet.__doc__)
setattr(ResNet3DV2, '__doc__', ResNet.__doc__)
