...
def block2(x, filters, kernel_size=3, stride=1,
    ...
    x = layers.Conv3D(filters, 1, strides=1, use_bias=False,
    ...
    x = layers.Conv3D(filters, kernel_size, strides=stride,
...
def block3(x, filters, kernel_size=3, stride=1, groups=32,
    ...
    x = layers.Conv3D(filters, 1, use_bias=False, trainable=False,
    ...
    def stack_fn(x):
        ...
        for depth_i in range(1, len(num_of_block)):
            x = stack1(x, base_channel*(2**(depth_i+1)), num_of_block[depth_i], 
                        name='conv{}'.format(depth_i+2))
        return x
    return ResNet(stack_fn, False, True, 'resnet_3D_{}'.format(base_channel),
                  include_top, weights,
