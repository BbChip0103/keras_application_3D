...
def DenseNet(blocks,
    ...
    x = layers.Conv3D(first_num_of_conv, 7, strides=2, use_bias=False, name='conv1/conv')(x)
    ...
    block_i = 0
    x = dense_block(x, blocks[block_i], growth_rate=growth_rate, 
                    name='conv{}'.format(block_i+2))
    for block_i in range(1, len(blocks)):
        x = transition_block(x, 0.5, name='pool{}'.format(block_i+2))
        x = dense_block(x, blocks[block_i], growth_rate=growth_rate, 
                        name='conv{}'.format(block_i+3))
...
def CustomDenseNet(blocks,
    ...
    block_i = 0
    x = dense_block(img_input, blocks[block_i], growth_rate=growth_rate, 
                    name='conv{}'.format(block_i+2))
    for block_i in range(1, len(blocks)):
        x = transition_block(x, 0.5, name='pool{}'.format(block_i+2))
        x = dense_block(x, blocks[block_i], growth_rate=growth_rate, 
    ...
    # Create model.
    model = models.Model(inputs, x, name='densenet_custom')

    return model


def CustomDenseNet3D(include_top=True,
                    weights=None,
                    input_tensor=None,
                    input_shape=None,
                    pooling=None,
...
def DenseNet3D(include_top=True,
                weights=None,
                input_tensor=None,
                input_shape=None,
                pooling=None,
